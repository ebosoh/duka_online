...
function addItemRow() {
  ...
  newRow.innerHTML = `
    <div>
      <label class="item-row-label">Item Name (e.g. Denim Jacket)</label>
      <input type="text" class="form-input buyer-item-name" placeholder="e.g. Denim Jacket" required>
    </div>
    <div>
      <label class="item-row-label">Agreed Price (KES)</label>
      <input type="number" class="form-input buyer-item-price" placeholder="e.g. 3200" oninput="updateCheckoutPricing()" required>
    </div>
    <button type="button" class="remove-item-btn" onclick="removeItemRow(this)">✕</button>
  `;
  ...
  trackEvent("checkout_item_added", { totalItems: rows.length });
...
function removeItemRow(btn) {
  ...
  const rows = container.querySelectorAll(".item-row");
  if (rows.length === 1) {
    const removeBtn = rows[0].querySelector(".remove-item-btn");
    if (removeBtn) {
      removeBtn.style.visibility = "hidden";
    }
  }

  trackEvent("checkout_item_removed", { totalItems: rows.length });
  updateCheckoutPricing();
}

function resetBuyerItemsList() {
  const container = document.getElementById("checkout-items-container");
  if (!container) return;
  container.innerHTML = `
    <div class="item-row">
      ...
      <button type="button" class="remove-item-btn" onclick="removeItemRow(this)" style="visibility: hidden;">✕</button>
    </div>
  `;
}
...
function updateCheckoutPricing() {
  ...
  const priceInputs = document.querySelectorAll(".buyer-item-price");
  priceInputs.forEach(input => {
    totalProductPrice += parseFloat(input.value) || 0;
  });
  ...
  const breakdownProdPrice = document.getElementById("breakdown-prod-price");
  if (breakdownProdPrice) breakdownProdPrice.textContent = `KES ${totalProductPrice.toLocaleString()}`;
  
  const breakdownDeliveryFee = document.getElementById("breakdown-delivery-fee");
  if (breakdownDeliveryFee) breakdownDeliveryFee.textContent = `KES ${deliveryFee.toLocaleString()}`;
  
  const breakdownTotalPrice = document.getElementById("breakdown-total-price");
  if (breakdownTotalPrice) breakdownTotalPrice.textContent = `KES ${totalPrice.toLocaleString()}`;
}
...
function triggerBuyerCheckout(event) {
  ...
  const nameInputs = document.querySelectorAll(".buyer-item-name");
  const priceInputs = document.querySelectorAll(".buyer-item-price");
  ...
  nameInputs.forEach((input, index) => {
    ...
    const priceVal = parseFloat(priceInputs[index].value) || 0;
    
    if (!nameVal || priceVal <= 0) {
      hasEmptyFields = true;
    } else {
      itemNamesArray.push(nameVal);
      totalProductPrice += priceVal;
    }
  });

  if (!name || !contact || !sellerSelect.value || !hubId || hasEmptyFields || itemNamesArray.length === 0) {
    showToast("Please fill in all Checkout details with valid prices!", "error");
    return;
  }

  const seller = ACTIVE_SELLERS.find((s) => s.id === sellerSelect.value);
  const hub = COLLECTION_POINTS.find((h) => h.id === hubId);
  const combinedItemName = itemNamesArray.join(" + ");
  ...
  pendingSTKPush = {
    ...
    productName: combinedItemName,
    productId: "custom",
    productPrice: totalProductPrice,
    deliveryFee: hub.fee,
    totalPaid: totalProductPrice + hub.fee,
...
function triggerReportPrint(timeframe) {
  ...
  // Filter based on roles for privacy/security
  let scopeTitle = "SYSTEM LOGS";
  if (currentRole === "seller") {
    filtered = filtered.filter((t) => t.merchantPhone === activeSellerChannel);
    const activeSeller = ACTIVE_SELLERS.find(s => s.PochiPhone === activeSellerChannel);
    scopeTitle = activeSeller ? activeSeller.name.toUpperCase() : "MERCHANT CONTROL";
  } else if (currentRole === "courier") {
    const authCourierStr = localStorage.getItem("duka_auth_courier");
    if (authCourierStr) {
      const courier = JSON.parse(authCourierStr);
      filtered = filtered.filter((t) => t.matched && t.collectionPoint === courier.name);
      scopeTitle = courier.name.toUpperCase();
    }
  }
